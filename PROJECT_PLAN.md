...
*   **Этапы:**
    *   **Этап 1 (Планируется):** Создать эталонную "шаблонную" Google-таблицу, которая будет служить образцом для всех новых пользователей.
    *   **Этап 2 (Планируется):** Реализовать функцию `onboardUser(chatId)`, которая будет создавать персональную папку пользователя в Google Drive и копировать в нее шаблонную таблицу при первом запуске бота.
    *   **Этап 3 (Планируется):** Интегрировать `onboardUser` в команду `/start`, чтобы обеспечить автоматическое создание инфраструктуры для новых пользователей.
    *   **Этап 4 (Планируется):** Полностью переработать функции в `6_sheets_api.js` для работы с персональными таблицами пользователей, получая ID таблицы из `PropertiesService` по `chatId`.
    *   **Этап 5 (Планируется):** Мигрировать существующие данные пользователей (если необходимо) в новую структуру.
