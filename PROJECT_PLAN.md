...
### 🔵 Задача 7: Реализовать генератор персонализированного меню

*   **Приоритет:** Высокий
*   **Ответственный:** Gemini
*   **Описание:** Создать ключевую AI-функцию бота — генератор персонализированного меню на основе рассчитанных КБЖУ пользователя. Бот должен подбирать комбинацию блюд из базы данных, чтобы составить сбалансированный рацион на день.
*   **Цель (SMART):** В течение следующей сессии реализовать и протестировать функцию `generatePersonalMenu(chatId)`, которая будет автоматически заполнять лист `Меню по дням` в персональной таблице пользователя на основе его КБЖУ и базы продуктов.
*   **Подзадачи:**
    *   **7.1.** В `0_setup.js` расширить функцию `setupProjectInfrastructure`, добавив создание листа `Продукты` в шаблонную таблицу. Этот лист будет содержать столбцы: `Блюдо`, `Калории`, `Белки`, `Жиры`, `Углеводы` (на 100г).
    *   **7.2.** Вручную заполнить лист `Продукты` в шаблонной таблице базовым набором из 15-20 блюд для каждого приема пищи (завтрак, обед, ужин, перекус).
    *   **7.3.** В `5_nutrition_ai.js` создать новую функцию `generatePersonalMenu(chatId)`.
    *   **7.4.** Реализовать в ней алгоритм подбора блюд, который будет стремиться к достижению дневной нормы КБЖУ пользователя с допустимым отклонением (например, ±10%).
    *   **7.5.** Записывать сгенерированное меню на 7 дней вперед в лист `Меню по дням` в персональной таблице пользователя.
    *   **7.6.** В `1_main.js` после успешного расчета КБЖУ (`triggerNutritionCalculation`) добавить кнопку с предложением "Сгенерировать мое меню".
    *   **7.7.** Протестировать весь цикл: расчет КБЖУ -> генерация меню -> запись в таблицу -> отображение меню пользователю.
...
### ✅ Глобальная Задача 3: Переход на персональную архитектуру данных

*   **Статус:** Реализовано, на тестировании
*   **Описание:** Фундаментальный рефакторинг системы хранения данных. Вместо использования общих `PropertiesService` и одной таблицы, для каждого нового пользователя будет автоматически создаваться персональная инфраструктура: отдельная папка в Google Drive и личная копия Google-таблицы для хранения всех его данных (настройки, меню, логи и т.д.).
*   **Этапы:**
    *   **Этап 1 (Выполнено):** Создана эталонная "шаблонная" Google-таблица.
    *   **Этап 2 (Выполнено):** Реализована функция `onboardUser(chatId)`.
    *   **Этап 3 (Выполнено):** `onboardUser` интегрирована в команду `/start`.
    *   **Этап 4 (Выполнено):** Функции в `6_sheets_api.js` переработаны для работы с персональными таблицами.
    *   **Этап 5 (На тестировании):** Проверить создание и работу с персональной инфраструктурой для новых пользователей.
