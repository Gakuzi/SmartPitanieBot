...
/**
 * Унифицированная обертка, возвращающая объект с полем text
 * и поддержкой expectJson ответа. Используется для согласования вызовов.
 */
function callGeminiStructured(prompt, expectJson = false) {
  ...
  if (expectJson) {
    try {
      ...
      if (parsed.response) {
        return { text: parsed.response, buttons: parsed.buttons || [] };
      }
      return { text: raw };
    } catch (e) {
      return { text: raw };
