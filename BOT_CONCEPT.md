...
## VI. Настройка Telegram Webhook

Чтобы Telegram мог отправлять обновления вашему боту, необходимо настроить вебхук — специальный URL, на который Telegram будет присылать все сообщения и команды.

### Как это работает:

1.  **Развертывание проекта:** Каждый раз, когда вы вносите изменения в код Google Apps Script, вам нужно делать "новое развертывание" (Deployment).
2.  **Получение URL:** После каждого развертывания вы получаете уникальный URL веб-приложения. **Этот URL меняется при каждом новом развертывании.**
3.  **Установка вебхука:** Вы должны сообщить Telegram этот новый URL.

### Пошаговая инструкция:

1.  **Разверните ваш проект:**
    *   В редакторе Google Apps Script нажмите **"Развертывание" (Deploy) -> "Новое развертывание" (New deployment)**.
    *   Убедитесь, что в настройках выбран тип **"Веб-приложение" (Web app)** и в поле **"Кто имеет доступ" (Who has access)** установлено **"Все" (Anyone)**.
    *   Нажмите кнопку **"Развернуть" (Deploy)**.

2.  **Скопируйте URL веб-приложения:**
    *   В появившемся окне скопируйте **URL веб-приложения (Web app URL)**. Он выглядит примерно так: `https://script.google.com/macros/s/ВАШ_ID/exec`.

3.  **Сформируйте и выполните запрос на установку вебхука:**
    *   Откройте новую вкладку в браузере.
    *   Вставьте в адресную строку следующую ссылку, заменив `<ВАШ_ТОКЕН>` на токен вашего бота, а `<НОВЫЙ_URL_СКРИПТА>` на URL, который вы скопировали на предыдущем шаге:
      ```
...
4.  **Проверьте результат:**
    *   Если все сделано правильно, вы увидите в браузере ответ от Telegram, подтверждающий успешную установку:
      ```json
      {"ok":true, "result":true, "description":"Webhook was set"}
      ```

**Важно:** Эту процедуру необходимо повторять **каждый раз** после внесения изменений в код и нового развертывания проекта. В проекте предусмотрен специальный диалог "Управление вебхуком", который упрощает этот процесс, но понимание ручного способа необходимо для отладки.
